...
def _run_case(index_file: Path, case: dict[str, object]) -> list[dict[str, str]]:
    return search(index_file, str(case["query"]), limit=int(case.get("limit", 10)))
...
def test_real_corpus_lookup_quality(real_index_file: Path, case: dict[str, object]) -> None:
    results = _run_case(real_index_file, case)
...
def test_mini_corpus_lookup_quality(mini_index_file: Path, case: dict[str, object]) -> None:
    results = _run_case(mini_index_file, case)
    category = str(case["category"])
...
def test_mini_fulltext_ranking(mini_index_file: Path) -> None:
    results = search(mini_index_file, "sharedterm", limit=10)
